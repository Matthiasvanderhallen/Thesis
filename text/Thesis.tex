...
label=code:DictionaryFunctorAlternative, morekeywords={where}]
functor DictionaryFn(KeyStruct:EQUAL) :> DICTIONARY where type key = KeyStruct.t =
    struct
        type key = KeyStruct.t
...
(However, they are not first-class values: they are bound to names using the special \lsttext{structure} keyword, and can only be returned by functors, not by any other expression.)
%todo: not by any Core language expression.
In the high-level language, structures can simply be represented by their name.
%\todo{remark: It is possible to compile away functors (And signatures). See: M. Elsman, Static Interpretation of Modules}
Compilation to the low-level language has several distinct possibilities of handling structures and functors:
\begin{itemize}
...
When is the structure resulting from a functor considered to be part of the SPM, and when is it part of the insecure context? This is discussed in \myref{sec}{sec:SecurityStatus}.

The second point, discussed in \myref{sec}{sec:LowLevelRepresentation} is whether the representational difference of structures when passing them as an argument to a functor in the high-level language and the low-level language can be exploited to compromise the \emph{SPM} and break contextual equivalence.
...
The structure that results from a functor has the same security status as the functor that is applied itself.
To see why this is the logical choice, consider the example of a dictionary in \myref{lst}{code:DictionaryFunctor}, and the two possible sources for the functor code:
\begin{description}
\item[Functor in secure code:] Suppose \lsttext{DictionaryFn} is defined in secure code.
...
\item[Functor in insecure code:] Suppose \lsttext{DictionaryFn} is defined in insecure code.
...
In order to pass structures to functors as an argument, structures are represented in the low-level language as a tuple of structure name, and a list of pointers that represent the structure values\todo{Explain the need of trimming, and solve the corresponding trimming problem}.
...
%todo: possible variant: this generic entry point takes all arguments and 'wraps around' the function that is to be called.
