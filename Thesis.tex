...
Since \mbox{MiniML} uses lexical scoping, functions can be defined as shown in Listing \ref{code:LexicalScopingExample}. The function \lsttext{addCurried} takes an argument \lsttext{x} and as a result returns another function, which makes it a higher order function. The function \lsttext{innerFunction} has access to the non-local or free variable \lsttext{x}, even though it is not defined in the local scope.
...
The two functions shown in Listing~\ref{code:CopyFreeVariables} are contextually equivalent in \mbox{MiniML}, but whose compiled versions are not contextually equivalent if the free-variables from untrusted code are not copied to the trusted memory. An attack can be set up that distinguishes a module using \lsttext{generateClosure1} from one using \lsttext{generateClosure2} as follows: after calling the closure, the context forces execution to temporarily revert to the unsafe code, for example using a context switch in a multithreaded environment. The context uses this opportunity to change the value of \lsttext{freevar} in the unprotected memory. If the implementation of \lsttext{generateClosure1} was used, then the result of the closure application depends on when exactly execution was temporarily switched to the attackers' context. If this happened after copying the value of \lsttext{freevar} to \lsttext{b}, which is saved in protected memory, then the result of the closure will not reflect the change of value of \lsttext{freevar} by the attackers' context. If the execution switch happens before this copying of \lsttext{freevar} into protected memory, then the result will be computed using the changed value for \lsttext{freevar}.
In contrast, and thereby breaking contextual equivalence, a version using  \lsttext{generateClosure2} never copies the value of \lsttext{freevar} into protected memory, and therefore the result of the closure will allways be computed using the changed value of \lsttext{freevar}. This breaks contextual equivalence of the compiled code.
...
%what if the free-variable IS a reference. Then we must follow the reference chain all the way?!
