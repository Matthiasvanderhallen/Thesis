...
Because this is only necessary when functions are called from the unprotected code, these security measures are taken care of by a low-level stub that wraps around the low-level translation of the high-level publicly available function.
...
The \mbox{MiniML} language allows a programmer to define his own types using the type keyword.
Taking another look at the Dictionary example of \myref{lst}{code:DictionaryStructureExample}, this happens in line 3.
The module's internal representation of a dictionary is a list of string pairs, but clearly this is some internal choice that the programmer does not want to make explicit, which is why the type synonym \lsttext{dictionary} is kept opaque. 
This can be seen in \myref{lst}{code:SignatureDictionaryExample}, line 3, where the external specification of a type is not revealed to be a list of string pairs.
...
It also follows that no stub nor entry function is necessary for the \lsttext{rand} function.

In its body, it returns the value of the \lsttext{seed}.
