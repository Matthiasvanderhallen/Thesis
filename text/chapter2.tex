...
First the syntax of \MiniML\ within which a program can be defined will be introduced. Next, the typing rules that a correct program must adhere to is shown. Lastly, the operational semantics determine what a correct program must do once it runs.
\section{MiniML}
\subsection{Syntax}
\newcommand{\longspace}{\;\;\;\;\;\;}
\newcommand{\inlinecode}{\texttt}
First we introduce the syntax of a \MiniML\ program, as seen in \myref{fig}{fig:Syntax}.
Like ML, the \MiniML\ syntax is composed of two parts: the \emph{Core} language, and a \emph{Module} language\cite{Milner:1997:DSM:549659}. 
These two languages, \emph{Core} and \emph{Module} each have their corresponding valid \emph{expression}s.
...
The \emph{Core} language expresses the manipulation of values and execution of functions, for example implementing small algorithms or control logic.
The \emph{Module} language expresses how these small parts of \emph{Core} expressions can be `glued together' or \emph{composed} into larger, working programs.
The \emph{Module} language introduces namespaces and encapsulation into ML. 
...
Having defined the syntax for \MiniML, this section formalizes the static semantics, also called its \emph{type system}.
