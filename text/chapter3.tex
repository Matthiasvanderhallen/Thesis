...
\item If the implementation of \lsttext{generateClosure1} was used, then the result of the closure application depends on when exactly execution was temporarily switched to the attacker's context.
If this happened after copying the value of \lsttext{freevar} to \lsttext{b}, which is saved in protected memory, then the result of the closure will not reflect the change of value of \lsttext{freevar} by the attacker's context.
...
Therefore the result of the closure will always be computed using the changed value of \lsttext{freevar}.
\end{enumerate}
\end{attack}
...
%To do: what if the free-variable IS a reference. Then we must follow the reference chain all the way?! No, we follow high-level semantics, and Ref's make MiniML storage aware. ≈If a Reference to storage is created by the attacker's context, and this reference is passed as a parameter, it is okay for the attacker to change the value in the storage location, as long as the location remains the same. 
...
\item Copy the parameters provided by the attacker's context in order to prevent \emph{call-by-value attacks} as shown in \myref{lst}{code:CopyFreeVariables}.
...
structure StringEqual: EQUAL = struct
        type t = string
        fun equal t1 t2  = case String.compare(t1,t2)
                                of EQUAL => true
                                | _ => false
    end
functor DictionaryFn (KeyStruct:EQUAL) :> DICTIONARY where type key = KeyStruct.t = struct
...
\begin{figure}[htb]
    ...
    \caption[Type Representations For Meta-frames]{This figure shows the representation of all different types \label{fig:TypeRepresentations}}
\end{figure}
...
For these expressions, the compiler must produce the structural typing checks so that they check:%Todo: Nalezen, is dit duidelijk genoeg?
\begin{enumerate}
...
Worse, the attacker's context is not even allowed to know the exact location \cmath{loc_{frame}} of these secure frames.
...
functor F(ArgStruct:ArgSig) :> OutputSig = struct
        val x = ArgStruct.f x
    end
...
structure StrA :> OutputSig = struct
        val x = Arg1.f x
    end
structure StrB = F(Arg2)
\end{lstlisting}
...
\centering
\includegraphics[scale=0.45]{img/Simple.png}
\caption{Execution flow in simple \MiniML. \label{fig:callstackcomparisonsimple}}
\end{subfigure}
\begin{subfigure}{0.55\textwidth}
\centering
\includegraphics[scale=0.45]{img/Advanced.png}
