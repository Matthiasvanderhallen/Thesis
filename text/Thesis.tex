...
For example, a problem with the dictionary example in Chapter 1, \myref{lst}{code:DictionaryStructureExample}, is that it is not really type independent. 
...
However, they are not first-class values: they are bound to names using the special \lsttext{structure} keyword, and can only be returned by functors, not by any Core language expression.
%todo explain
...
\myref{sec}{sec:LowLevelRepresentation} expands on the representation and discusses whether this different representation can be exploited by an attacker.
...
Suppose \lsttext{DictionaryFn} is defined in secure code.
...
In order to pass structures to functors as an argument, structures are represented in the target language as a tuple of structure name, and a list of pointers that represent the structure values\todo{Explain the need of trimming, and solve the corresponding trimming problem}.
...
When functor code is called with a reference to the frame in secure memory, the functor code would be able to read the the frame directly, because the functor is part of the \emph{SPM}. It could then call any value of the structure using the pointer provided by the frame.
