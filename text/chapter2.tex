...
%Todo: Nalezen, is dit duidelijk genoeg?

Whether or not a structure's principal signature matches with a signature \cmath{Sig}, depends only on the structural characteristics \cmath{Sig} requires, and not on whether the structure was explicitly ascribed with \cmath{Sig}.
For this reason, signature matching is a form of \emph{structural typing}, as opposed to \emph{nominal typing}~\cite{Pierce:Adv}.
...
%\todo{How to express the substitution of all references to argument placeholder module names to the module's names given at execution}
...
\subsection{Syntax}
In \myref{fig}{fig:LLVMSyntax}, the reduced syntax of LLVM IR is given, with some small changes from the work of Jianzhou Zhao et al.~\cite{Zhao:2012:FLI:2103656.2103709}.\begin{figure}[!htb]
\begin{align*}
\begin{aligned}
...
The result of the translation is expected to run as an \emph{SPM} on a low-level machine model as used in~\cite{Agten:2012:SCM:2354412.2355247,Patrignani}.
...
\input{formalization}
\vspace{-1.5em} %Todo: LaTeX voodoo
...
An implementation of this formal compiler was written in Scala.
This secure compiler takes an abstract syntax tree (\emph{AST}) representing a \MiniML\ program as its input.
The nodes of this \emph{AST} correspond to \MiniML's syntactical constructs, as specified in \myref{sec}{sec:ch2syntax}.
It assumes that the program represented by this \emph{AST} is correctly typed, i.e. it has passed a type check as specified by the type system in \myref{sec}{sec:MLTypeSystem}.

As a result of the compilation, it outputs a file with extension .ll, containing the LLVM IR translation of the \MiniML\ program.
An example output of this compiler was shown in \myref{sec}{sec:TranslatedExample}.
