...
This chapter extends the capabilities of the subset of the ML language. \myref{sec}{sec:HOF} introduces \emph{higher-order functions} and discusses how they can be compiled securely. Afterwards, \myref{sec}{sec:Functors} explains \emph{functors} and their addition to the high-level language. This chapter continues by showing how the concepts learned while adding higher-order functions can aid the low-level implementation of functors in \myref{sec}{sec:Functors}.
...
%Since \mbox{MiniML} uses lexical scoping, functions can be defined as shown in \emph{Listing} \ref{code:LexicalScopingExample}. The function \lsttext{addCurried} takes an argument \lsttext{x} and as a result returns another function. Therefore, it is a higher-order function. The function \lsttext{innerFunction} has access to the non-local or free variable \lsttext{x}, even though it is not defined in the local scope.
...
A closure consists of a simple function reference, together with a referencing environment which contains all the different free variables that are available through the lexical scoping rules.
...
Even when no callbacks are available, in a more powerful language providing a multithreaded environment such an execution passing can be forced using a context switch.
...
After all, the high-level language doesn't allow the memory location of the parameter that was passed to be changed which is a necessary access right to perform step two of the attack.
...
These same measures are of course necessary when working within the context of closures.
...
The only way to interact with these secure free variables is by passing them as parameters to modifying functions from within the \emph{SPM}.
This analysis shows that the added power of closures does not require any new security measures for this interaction between \emph{SPM} and the attackers context.
...
If an SPM would pass a closure generated inside its body to the untrusted code, the untrusted code could change the value of the function pointer and inspect or change the values saved in the referencing environment.
...
As a result, the closure entity itself should not be passed across the boundary between the SPM and the trusted code.
Instead, the closure should be masked, and its corresponding pointer in the masking map is passed to the untrusted code.
In order to later execute the closure, the SPM should offer an \emph{closure-evaluation entry point}.
This entry point first takes a pointer to a closure in the masking map.
Then, because it is located within the SPM, the entry point is able to jump to the function pointer specified by the closure and run the function code.

\subsubsection{The Closure-Evaluation Entry Point}

Because closures can be passed freely it is possible for insecure code within the attackers context to make use of the closure, and execute the underlying function.
Execution of a closure is also called the \emph{application} of the closure. 
