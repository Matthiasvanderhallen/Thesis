...
The dictionary structure as given in \myref{lst}{code:DictionaryStructureExample} is not really type independent since only values of type string are allowed to be used as keys.

Ideally however, how a dictionary works would be described in a generic way that doesn't care whether the key is of type string, int or any other type.
...
\smallskip
Choosing the second option means that all values of functors are implemented as stubs that take a reference to a frame. 
These stubs are added as \emph{entry points} to the SPM.
The stubs can implement any generic code, and use the information inside the frame to call any necessary values defined by the argument structure.
