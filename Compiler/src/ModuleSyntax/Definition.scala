...
case class TypeDefinition (ident:Ident, tyvars:Int, definition: Type) extends Definition {
  def translate(trans:Translation, structure:Structure, signature:Signature):Translation = {
    ...
    if(declaration.isEmpty){
      ...
      translation.types = translation.types.concat("%" + structure.ident + "." + this.ident + " = type {" + this.definition + "}; " + translation.opaqueTypeCounter + "\n") //Todo wat met this dependencies
      translation.opaqueTypeMapping = translation.opaqueTypeMapping + ((structure.ident.value + "." + this.ident.value) -> translation.opaqueTypeCounter)
