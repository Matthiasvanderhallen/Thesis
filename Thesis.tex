...
&|\;\mathit{fix\;e} \\
\\
\text{Access Path } \mathit{path} \; ::= \; &\mathit{M}\\
&|\;M_{1}(\overline{\mathit{M_{2}}})\\
&|\;\mathit{this}\\
\\
\text{Module } \mathit{Mod} ::= \; &\lbrace \Sigma, \overline{\mathit{d}}\rbrace^{M}\\
&|\;\lbrace \Sigma, \overline{\Sigma}, \overline{d} \rbrace^{M} \\
\end{aligned}
\begin{aligned}
\text{Signature } \Sigma \; ::=\; & \overline{\Delta}^{S}\\
\\
...
&| \; (x:\sigma),\Gamma \\
&| \; (M=\lbrace \Sigma,\overline{d}\rbrace^{M}), \Gamma \\
...
\subsection{Helpful relations}
Some relationships can be defined over type-schemes, types and contexts: For example,  type-scheme specialization and type-scheme generalization.
...
\\
\tag{T-ModVarOther}
&\frac{\Gamma \vdash \lbrace\Sigma,d\rbrace^{M_{name}} \longspace id:\tau \in \Sigma}
{\Gamma \vdash \mathit{M_{name}.id} : \tau} \\
\\
\tag{T-FunctorVar}
&\frac{\Gamma \vdash \lbrace \Sigma_{1}, \overline{\Sigma_{3}}, \overline{d}\rbrace
\longspace
\overline{\Gamma \vdash \lbrace \Sigma_{2}, \overline{d_{2}}\rbrace}
\longspace
\overline{\Sigma_{3} \succeq \Sigma_{2}}}
{\Gamma \vdash \mathit{M_{1}(\overline{\lbrace \Sigma_{2}, \overline{d_{2}}\rbrace}).id}:\tau} \\
\\
\tag{T-Module}
&\frac{\lbrace\Sigma,\overline{d}\rbrace^{M} \in \Gamma
\longspace
\emptyset \vdash d \rightarrow \Gamma' \longspace \Gamma' \vdash \Sigma}
{\Gamma \vdash \lbrace\Sigma,\overline{d}\rbrace^{M}} \\
\\
\tag{T-Functor}
&\frac{\lbrace\Sigma,\overline{\Sigma_{2}},\overline{d}\rbrace^{M} \in \Gamma
\longspace
\emptyset \vdash d \rightarrow \Gamma' \longspace \Gamma' \vdash \Sigma}
{\Gamma \vdash \lbrace\Sigma,\overline{\Sigma_{2}},\overline{d}\rbrace^{M}} \\
\\
